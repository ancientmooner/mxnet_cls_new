...
struct DefaultImageAugmentParam : public dmlc::Parameter<DefaultImageAugmentParam> {
  ...
  float max_img_size;
  /*! \brief color jittering by pca lightning */
  float pca_alpha;
  ...
  DMLC_DECLARE_PARAMETER(DefaultImageAugmentParam) {
    ...
    DMLC_DECLARE_FIELD(min_img_size).set_default(0.0f)
        .describe("Set the minimal width and height after all resize and"
                  " rotate argumentation  are applied");
    DMLC_DECLARE_FIELD(pca_alpha).set_default(0.0f)
        .describe("Set the alapha in"
                  " pca color jittering as in AlexNet is applied");
...
class DefaultImageAugmenter : public ImageAugmenter {
 public:
  ...
  void SatuationJitter(cv::Mat &res, float rand_jitter) {
    cv::Mat gray;
    cvtColor(res, gray, CV_BGR2GRAY);
    cvtColor(gray, gray, CV_GRAY2BGR);
    res = res * (rand_jitter + 1) - gray * rand_jitter; 
  }

  void BrightnessJitter(cv::Mat &res, float rand_jitter) {
    res = res * (rand_jitter + 1); 
  }

  void ContrastJitter(cv::Mat &res, float rand_jitter) {
    cv::Mat gray;
    cvtColor(res, gray, CV_BGR2GRAY);
    cv::Scalar mean_val = cv::mean(gray);
    res = res * (rand_jitter + 1) - (float)mean_val[0] * rand_jitter; 
  }
  ...
  cv::Mat Process(const cv::Mat &src, std::vector<float> *label,
    ...
    if ((param_.min_random_scale != 1.0 || param_.max_aspect_ratio != 0.0f) && (param_.rotate == 0 || param_.max_shear_ratio == 0.0f))
    {
      ...
      if (rand_cnt >= 10){
        int hs;
        int ws;
        if ((float)res.rows / param_.data_shape[1] < (float)res.cols / param_.data_shape[2]) {
          hs = res.rows;
          ws = res.rows * param_.data_shape[2] / (float)param_.data_shape[1];
        }
        else{
          ws = res.cols;
          hs = res.cols * param_.data_shape[1] / (float)param_.data_shape[2];
        }

        int x1 = (int)(0.5 * (res.cols - ws));
        int y1 = (int)(0.5 * (res.rows - hs));
    ...
    cv::Mat res_float;
    res.convertTo(res_float, CV_32F, 1.0/255.0, 0);

    //PCA jittering
    if (param_.pca_alpha !=0) {
      std::normal_distribution<float> rand_gauss(0, param_.pca_alpha);
      float alpha1 = rand_gauss(*prnd);
      float alpha2 = rand_gauss(*prnd);
      float alpha3 = rand_gauss(*prnd);

      float r_jitter = -0.5675 * 0.2175 * alpha1 + 0.7192 * 0.0188 * alpha2 + 0.4009 * 0.0045 * alpha3; 
      float g_jitter = -0.5808 * 0.2175 * alpha1 - 0.0045 * 0.0188 * alpha2 - 0.8140 * 0.0045 * alpha3; 
      float b_jitter = -0.5836 * 0.2175 * alpha1 - 0.6948 * 0.0188 * alpha2 + 0.4203 * 0.0045 * alpha3;
      cv::Scalar scalar(b_jitter, g_jitter, r_jitter);
      res_float = res_float + scalar; 
    }
    
    //cv::Mat res_save;
    //res_float.convertTo(res_save, CV_8U, 255.0, 0);
    //cv::imwrite("pca_jittering.png", res_save);

    // color space augmentation
    if (param_.random_h != 0 || param_.random_s != 0 || param_.random_l != 0) {
      std::uniform_real_distribution<float> rand_uniform(0, 1);
      float h = rand_uniform(*prnd) * param_.random_h * 2 - param_.random_h;
      float s = rand_uniform(*prnd) * param_.random_s * 2 - param_.random_s;
      float l = rand_uniform(*prnd) * param_.random_l * 2 - param_.random_l;
      h = h / 255.0;
      s = s / 255.0;
      l = l / 255.0;
      std::uniform_int_distribution<int> rand_int(0, 5);
      int order_choice = rand_int(*prnd);
      if (0 == order_choice){ 
        SatuationJitter(res_float, s);
        BrightnessJitter(res_float, l);
        ContrastJitter(res_float, h);
      }
      else if(1 == order_choice){
        SatuationJitter(res_float, s);
        ContrastJitter(res_float, h);
        BrightnessJitter(res_float, l);
      }
      else if(2 == order_choice){
        ContrastJitter(res_float, h);
        SatuationJitter(res_float, s);
        BrightnessJitter(res_float, l);
      }
      else if(3 == order_choice){
        ContrastJitter(res_float, h);
        BrightnessJitter(res_float, l);
        SatuationJitter(res_float, s);
      }
      else if(4 == order_choice){
        BrightnessJitter(res_float, l);
        SatuationJitter(res_float, s);
        ContrastJitter(res_float, h);
      }
      else if(5 == order_choice){
        BrightnessJitter(res_float, l);
        ContrastJitter(res_float, h);
        SatuationJitter(res_float, s);
      }
      //res_float.convertTo(res_save, CV_8U, 255.0, 0);
      //cv::imwrite("color_aug.png", res_save);
    ...
    res_float = res_float * 255.;
    //res_float.convertTo(res_save, CV_8U, 1.0, 0);
    //cv::imwrite("before_mean.png", res_save);
    return res_float;
  }
