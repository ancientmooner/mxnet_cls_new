...
template<typename DType>
inline void ImageRecordIOParser3<DType>::Init(
  ...
  CHECK(param_.path_imgrec.length() != 0)
      << "ImageRecordIter3: must specify image_rec";

  if (param_.verbose) {
    LOG(INFO) << "ImageRecordIOParser3: " << param_.path_imgrec
...
template<typename DType>
inline bool ImageRecordIOParser3<DType>::ParseNext(DataBatch *out) {
  if (overflow) {
    return false;
  }
...
template<typename DType>
template<int n_channels>
void ImageRecordIOParser3<DType>::ProcessImage(const cv::Mat& res,
  mshadow::Tensor<cpu, 3, DType>* data_ptr, const bool is_mirrored, const float contrast_scaled,
  const float illumination_scaled) {
  float RGBA_MULT[4] = { 0 };
  float RGBA_BIAS[4] = { 0 };
...
template<typename DType>
cv::Mat ImageRecordIOParser3<DType>::TJimdecode(cv::Mat image, int color) {
...
template<typename DType>
inline unsigned ImageRecordIOParser3<DType>::ParseChunk(DType* data_dptr, real_t* label_dptr,
...
// create mean image.
template<typename DType>
inline void ImageRecordIOParser3<DType>::CreateMeanImg(void) {
    if (param_.verbose) {
...
class ImageRecordIter3 : public IIterator<DataBatch> {
 public:
    ImageRecordIter3() : out_(nullptr) { }

    virtual ~ImageRecordIter3(void) {
      iter_.Destroy();
    }
 ...
 private:
    ...
    /* \brief parser */
    ImageRecordIOParser3<DType> parser_;
};

MXNET_REGISTER_IO_ITER(ImageRecordIter3)
...
.set_body([]() {
    return new ImageRecordIter3<real_t>();
    });

MXNET_REGISTER_IO_ITER(ImageRecordUInt8Iter_3)
...
.add_arguments(ListDefaultAugParams())
.set_body([]() {
    return new ImageRecordIter3<uint8_t>();
  });
}  // namespace io
}  // namespace mxnet
