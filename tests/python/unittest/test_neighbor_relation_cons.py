...
def consistent_check(key_shape, value_shape, kernel_size=7, num_group=32, test_bp=False):
    ...
    query_map = np.random.rand(*key_shape)
    ...
    data_shape_dict = {'value_data': value_shape, 'key_data': key_shape, 'query_data': key_shape, 'pos_weight_data': pos_weight_shape}
    ...
    for i_arg in range(len(programs[0].outputs)):
        ...
        err = np.sum(err)
        #print "dcn_v1", programs[0].outputs[i_arg].asnumpy()
        #print "dcn_v3", programs[1].outputs[i_arg].asnumpy()
...
if __name__ == '__main__':
    ...
    batch_size = 8
    key_shape=(batch_size, 256, 56, 56)
    value_shape=(batch_size, 64, 56, 56)
    kernel_size = 7
    num_group = 32
    consistent_check(key_shape, value_shape, kernel_size=kernel_size, num_group=num_group, test_bp=test_bp)
    
    key_shape=(batch_size, 512, 28, 28)
    value_shape=(batch_size, 128, 28, 28)
    kernel_size = 7
    num_group = 64
    consistent_check(key_shape, value_shape, kernel_size=kernel_size, num_group=num_group, test_bp=test_bp)
    
    key_shape=(batch_size, 1024, 14, 14)
    value_shape=(batch_size, 256, 14, 14)
    kernel_size = 7
    num_group = 128
    consistent_check(key_shape, value_shape, kernel_size=kernel_size, num_group=num_group, test_bp=test_bp)
    
    key_shape=(batch_size, 2048, 7, 7)
    value_shape=(batch_size, 512, 7, 7)
    kernel_size = 7
    num_group = 256
    consistent_check(key_shape, value_shape, kernel_size=kernel_size, num_group=num_group, test_bp=test_bp)
