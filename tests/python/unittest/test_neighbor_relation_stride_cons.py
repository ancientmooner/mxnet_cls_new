...
def neighbor_relation_v1(value_data, key_data, query_data, pos_weight, scale, batch_size, cur_height, cur_width, num_group=32, kernel_size=7, dilate=1, norm_method=0, sim_method=0, no_define_value=-100.0, stride=1, mask=None):
    ...
    if sim_method == 0:
        ...
        app_dot = mx.sym.broadcast_add(query_data_reshape, warp_key_data_reshape)
...
def neighbor_relation_v2(value_data, key_data, query_data, pos_weight, scale, batch_size, cur_height, cur_width, num_group=32, kernel_size=7, dilate=1, norm_method=0, sim_method=0, no_define_value=-100.0, stride=1):
    
    output_value = mx.sym.contrib.NeighborRelation(key=key_data, value=value_data, query=query_data, 
                    pos_weight=pos_weight, scale=scale, num_group=num_group, kernel_size=kernel_size, dilate=dilate, batch_step=32, norm_method=norm_method, sim_method=sim_method, no_define_value=no_define_value, dropout_ratio=0.0, stride=stride)
    #output_value = mx.sym.contrib.NeighborRelation(key=key_data, value=value_data, query=query_data, 
...
def consistent_check(key_shape, query_shape, value_shape, kernel_size=7, num_group=32, test_bp=False, stride=1):
    # param
    ...
    scale = 1
    neighbor_relation_out_v2 = neighbor_relation_v2(value_data, key_data, query_data, pos_weight_data, scale, batch_size, height, width, num_group=num_group, kernel_size=kernel_size, dilate=1, norm_method=0, sim_method=0, no_define_value=0.0, stride=stride)
    
    neighbor_relation_out_v1 = neighbor_relation_v1(t_value_data, t_key_data, t_query_data, t_pos_weight_data, scale, batch_size, height, width, num_group=num_group, kernel_size=kernel_size, dilate=1, norm_method=0, sim_method=0, no_define_value=0.0, stride=stride)
...
if __name__ == '__main__':
    ...
    key_shape=(batch_size, num_group * key_dim, in_size, in_size)
    query_shape=(batch_size, num_group * key_dim, out_size, out_size)
    value_shape=(batch_size, num_group * values, in_size, in_size)
    consistent_check(key_shape, query_shape, value_shape, kernel_size=kernel_size, num_group=num_group, test_bp=test_bp, stride=stride)
