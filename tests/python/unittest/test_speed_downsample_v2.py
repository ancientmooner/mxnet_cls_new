...
#@with_seed(1234)
def test_downsample_speed(sampler, data_shape, kernel_shape, repeat_num, profile):
    
    data = mx.sym.Variable('data')
    kernel = mx.sym.Variable('kernel')
    sym = sampler(data=data, kernel=kernel, backward_kernel=False, rescale=1, dilate=1)
    ctx = mx.gpu(0)
    exe = sym.bind(ctx=ctx, args={
        'data': mx.nd.normal(loc=0, scale=1, shape=data_shape, ctx=ctx),
        'kernel': mx.nd.normal(loc=0, scale=1, shape=kernel_shape, ctx=ctx)
        },
        args_grad = {
        'data': mx.nd.ones(data_shape, ctx=ctx),
        'kernel': mx.nd.ones(kernel_shape, ctx=ctx)
        })
...
if __name__ == '__main__':
    ...
    sampler_dict = {1: mx.sym.contrib.Downsample, 2: mx.sym.contrib.DownsampleV2}
    ...
    for stage, param in params.items():
        ...
        print('Stage:', stage)
        test_downsample_speed(sampler, data_shape, kernel_shape, repeat_num, args.p)
